...
- **Два режима:**
  - *сохранить только временные слои* — векторные слои в памяти и результаты «Обработки» из временной папки (вектор и растр) в выбранную папку. По умолчанию это папка `data` рядом с файлом проекта: если её нет, она создаётся. Можно выбрать и другую — для того же проекта выбор не сбрасывается;
...
- **Временные слои модуля Memory Layer Saver.** Этот модуль хранит объекты временных слоёв внутри файла проекта и загружает их при открытии. Если проект открыт без него (модуль не установлен или выключен), такие слои пустые. Сохранить их — значит заменить объекты пустыми файлами, поэтому в списке они видны серыми с пометкой «объекты не загружены — нужен Memory Layer Saver» и не сохраняются. Установите или включите Memory Layer Saver и откройте проект заново. Слои, пустые на самом деле, сохраняются как обычно.
...
- **Длинные пути сокращаются.** Если путь к файлу в `data_all` вышел бы длиннее 259 знаков (в Windows такие файлы не создаются и не открываются), сокращаются названия самых длинных подпапок — ровно настолько, чтобы путь поместился. Папки модуля (`external_links`, `raster`…) не трогаются, одна исходная папка получает одно короткое имя, разные папки с одинаковым началом — разные (`~2`). Работает на любой системе: `data_all` собирают, чтобы передать дальше. Если и так не помещается, файл кладётся прямо в `data_all`. Журнал пишет об этом у слоя.
...
2. Выберите режим: «Сохранить только временные слои» или «Собрать все слои в папку data_all» (с архивом или без), формат и при необходимости систему координат. Временные слои по умолчанию сохраняются в папку `data` рядом с файлом проекта, другую папку можно выбрать.
...
Скрипт сам находит QGIS в `/Applications`; другую сборку можно указать так: `QGIS_APP=/Applications/QGIS-3.44.app tests/run_tests.sh`.
