...
- **Два режима:**
  ...
  - *собрать все слои в папку data_all* — ещё и постоянные: файлы, базы (PostGIS, SpatiaLite), WFS, CSV, растры; по флажку «Сохранять структуру папок» — по исходным подпапкам, по флажку «Создать архив» — ещё и zip для заказчика (см. ниже). Онлайн-подложки (WMS, XYZ) и векторные тайлы сохранить нельзя — они видны в списке серыми с пояснением. Облака точек и сетки (mesh) собираются только со «структурой папок»: их файлы копируются как есть. Так же помечается слой, файл которого удалили с диска при открытом проекте: QGIS ещё показывает его на карте, но собрать его уже нельзя.
...
**Сохранять структуру папок** (флажок под режимом сборки, по умолчанию выключен). Файлы слоёв не переводятся в другой формат, а копируются как есть и раскладываются по тем же подпапкам (`PROJECT_DIR` — папка файла проекта):
...
- В выбранный в окне формат переводятся только слои в памяти, из баз данных и WFS (они ложатся в саму `data_all`, временные растры — в `data_all/raster`), а также VRT и слои, которые перепроецируются в другую систему координат. Такие слои ложатся на место исходного файла и сохраняют его имя: `мозаика.vrt` → `мозаика.tif`; слой из файла с несколькими слоями (GeoPackage, KML) — под своим названием.
