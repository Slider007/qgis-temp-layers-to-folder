...
- **Структура папок.** Флажок «Сохранять структуру папок» (в режимах «все слои» и «собрать проект»): файлы ложатся в такие же подпапки, как исходные, а не все в одну папку. Подпапки считаются от папки проекта: `Проект/Данные/Растры/dem.tif` → `<папка>/Данные/Растры/Рельеф.tif`. Для файлов вне папки проекта подпапки считаются от их общей папки, и её имя сохраняется: `D:/Архив/Топо/реки.shp` и `D:/Архив/Почвы/почвы.shp` → `<папка>/Архив/Топо/…` и `<папка>/Архив/Почвы/…`. Слои в памяти, временные и из баз данных ложатся в саму папку. В режиме «все слои в одном файле» подпапки получают только растры. По умолчанию выключено.
...
- **Структура папок** — с флажком «Сохранять структуру папок» `data_all` повторяет подпапки проекта: `Данные/Вектор/Опоры.shp` → `data_all/Данные/Вектор/Опоры.gpkg`. Картинки и шрифты всё равно лежат в `data_all/images/` и `data_all/fonts/`.
